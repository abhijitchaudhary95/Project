                                    ...
                                    @if($banner)
                                        @foreach($banner as $key =>$banner_info)
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="{{ $banner_info->link }}" target="_banner" class="btn-link">
                                                        {{ $banner_info->link }}
                                                    </a>
                                                </td>
                                                <td>
                                                   @if(file_exists(public_path().'/uploads/banner/'.$banner_info->image))
                                                ...
                                                <td>
                                                    <a href="{{ route('banner-edit',$banner_info->id) }}" style="border-radius: 50%" class="btn btn-success">
                                                        <i class="fa fa-pencil"> </i>
                                                    </a>
                                                    <form method="post" action="{{ route('banner-delete',$banner_info->id) }}" onsubmit="return confirm('Are you want to delete this banner')"  >
                                                        @csrf
                                                        @method('delete')
